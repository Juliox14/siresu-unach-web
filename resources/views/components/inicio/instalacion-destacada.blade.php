...
{{-- Ajuste móvil: py-20 se cambió a py-12 en celular --}}
<section class="relative py-12 md:py-20 overflow-hidden rounded-2xl group">
    ...
    <div class="relative z-10 container mx-auto px-6 md:px-12">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
            ...
            <div class="flex justify-center lg:justify-end order-2 lg:order-1 mt-4 lg:mt-0">
                @if($instalacionDestacada->imagen_portada)
                    <img src="{{ asset('storage/' . $instalacionDestacada->imagen_portada) }}"
                         ...
                         class="w-[85%] md:w-3/4 h-auto drop-shadow-2xl"
                         style="animation: float 6s ease-in-out infinite;">
                @endif
            </div>

            <div class="text-white order-1 lg:order-2 text-center lg:text-left space-y-5 md:space-y-6">

                <div>
                    {{-- Ajuste tipográfico escalable --}}
                    <h2 class="font-heading font-extrabold text-3xl md:text-4xl lg:text-5xl leading-tight tracking-tight text-white"
                    ...
                    <div class="h-1 w-16 md:w-20 bg-unach-dorado mt-4 rounded-full mx-auto lg:mx-0"></div>
                </div>

                <p class="font-poppins text-white/80 text-base md:text-lg leading-relaxed font-light">
                ...
                @if($instalacionDestacada->caracteristicas && count($instalacionDestacada->caracteristicas) > 0)
                    <ul class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 text-left">
                        @foreach($instalacionDestacada->caracteristicas as $bullet)
                            <li class="flex items-start lg:items-center justify-start gap-2 font-poppins text-sm text-white/90">
                                <span class="w-2 h-2 mt-1.5 lg:mt-0 bg-unach-dorado rounded-full shrink-0"></span>
                                <span>{{ $bullet }}</span>
                            </li>
                        @endforeach
                    </ul>
                ...
                {{-- El bloque de teléfono se quedó igual, ya estaba bien --}}
                @if($instalacionDestacada->telefono)
                    <div class="border-t border-white/20 pt-6 space-y-1">
                        ...
                        <p class="font-heading font-bold text-xl flex items-center justify-center lg:justify-start gap-2">
                            <x-heroicon-m-phone class="w-5 h-5 text-unach-dorado shrink-0" />
                            ...
                            @if($instalacionDestacada->extension)
                                <span class="font-poppins text-sm font-normal text-unach-dorado">
                                    Ext. {{ $instalacionDestacada->extension }}
                                </span>
                            @endif
                        </p>
                    </div>
                ...
                <div>
                    <a href="{{ route('instalaciones.index') }}#instalacion-{{ $instalacionDestacada->id }}"
                       class="inline-flex items-center gap-2 px-6 py-3 md:px-8 md:py-3 bg-unach-dorado text-unach-azul-oscuro font-poppins font-bold rounded-xl shadow-md hover:brightness-95 transition-all duration-300">
                        Conocer más
